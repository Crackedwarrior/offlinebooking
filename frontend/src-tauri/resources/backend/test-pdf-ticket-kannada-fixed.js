...
const kannadaFontPath = path.join(__dirname, 'node_modules', '@fontsource', 'noto-sans-kannada', 'files', 'noto-sans-kannada-kannada-400-normal.woff2');
...
function createPDFTicket(ticketData) {
  ...
  const outputPath = path.join(__dirname, 'temp', `ticket_kannada_fixed_${Date.now()}.pdf`);
  ...
  // Bold AND italic theater name (flowing, calligraphy-like) - using Kannada font
  ...
  doc.text(` ಸೀ.ನಂ:${ticketData.ticketId}/${ticketData.currentTime}`, 145, currentY - 5); // Position beside date with S.No in Kannada
  currentY += 15;
  ...
  doc.text(`ಸೀಟ್  : ${ticketData.seatInfo}`, 60, currentY + 32); // Aligned semicolon with CLASS
  ...
  currentY += 12;
  doc.text(`[ಟಿಕೆಟ್ ಬೆಲೆ: Rs.${ticketData.totalAmount.padStart(6)}]`, 60, currentY); // Using Rs. instead of ₹
  ...
  doc.fontSize(titleFontSize).font('Times-Bold'); // Same font family as theater name, no italic
  doc.text(`ಒಟ್ಟು: Rs.${ticketData.totalAmount}`, 55, priceStartY + 51, { width: 176, align: 'center' }); // Centered, prominent text
  ...
  return new Promise((resolve, reject) => {
    stream.on('finish', () => {
      console.log(`Kannada PDF ticket generated: ${outputPath}`);
      resolve(outputPath);
    });
