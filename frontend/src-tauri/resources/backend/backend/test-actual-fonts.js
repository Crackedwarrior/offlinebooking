...
function createPDFTicket(ticketData) {
  ...
  const outputFilePath = path.join(__dirname, 'temp', `test_actual_fonts_${Date.now()}.pdf`);
  ...
  // Test fonts that actually exist on the system
  const actualFonts = [
    'Arial',
    'Segoe UI',
    'Segoe UI Light',
  ...
  actualFonts.forEach((fontName, index) => {
    try {
      doc.font(fontName);
