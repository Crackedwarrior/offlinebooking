...
class PrinterSetup {
     ...
     */
    static async openPrinterProperties(printerName) {
        try {
     ...
     */
    static async openPrinterPreferences(printerName) {
        try {
    ...
    static async listPrinters() {
        try {
            ...
            return printerList.map((p) => p.Name).filter(Boolean);
        }
        catch (error) {
     ...
     */
    static async getPrinterInfo(printerName) {
        try {
            const { stdout } = await execAsync(`powershell -Command "Get-Printer -Name '${printerName}' | ConvertTo-Json"`, { windowsHide: true });
     ...
     */
    static async setDefaultPrinter(printerName) {
        try {
     ...
     */
    static async setupPrinter(printerName) {
