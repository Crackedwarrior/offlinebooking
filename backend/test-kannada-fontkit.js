...
async function createPDFTicket(ticketData) {
  ...
  const outputFilePath = path.join(__dirname, 'temp', `test_kannada_fontkit_${Date.now()}.pdf`);
  ...
  // Try to embed the Nudi font using fontkit
  let kannadaFont = 'Helvetica'; // fallback
  const nudiFontPath = 'C:\\Windows\\Fonts\\Nudi 01 e.ttf';
  
  try {
    // Read the font file
    const fontBuffer = fs.readFileSync(nudiFontPath);
    
    // Parse the font with fontkit
    const font = await fontkit.create(fontBuffer);
    
    // Check if font supports Kannada
    const hasKannada = font.characterSet && font.characterSet.some(range => 
      range.start <= 0x0C80 && range.end >= 0x0CFF
    );
    
    if (hasKannada) {
      console.log('✅ Font supports Kannada characters');
      // Embed the font in the PDF
      doc.font(fontBuffer);
      kannadaFont = fontBuffer;
      console.log('✅ Font embedded successfully');
    } else {
      console.log('❌ Font does not support Kannada characters');
    }
  } catch (error) {
  ...
  doc.fontSize(16).font(kannadaFont);
  ...
  doc.fontSize(10).font(kannadaFont);
  ...
  doc.fontSize(12).font(kannadaFont);
  ...
  doc.fontSize(8).font(kannadaFont);
  ...
  doc.fontSize(14).font(kannadaFont);
  ...
  doc.fontSize(7).font(kannadaFont);
  ...
  console.log(`Font used: ${kannadaFont === 'Helvetica' ? 'Helvetica (fallback)' : 'Nudi 01 e.ttf (embedded)'}`);
}
