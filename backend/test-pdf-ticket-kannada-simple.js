...
function createPDFTicket(ticketData) {
  ...
  const outputPath = path.join(__dirname, 'temp', `ticket_kannada_simple_${Date.now()}.pdf`);
  ...
  // Try to use a system font that supports Kannada
  try {
    // Try Arial Unicode MS which should support Kannada
    doc.fontSize(16).font('Arial Unicode MS');
  } catch (e) {
    // Fallback to Times-BoldItalic if Arial Unicode MS not available
  ...
  // Date and Show info - try Kannada font for Kannada text
  try {
    doc.fontSize(normalFontSize).font('Arial Unicode MS');
  } catch (e) {
  ...
  try {
    doc.fontSize(titleFontSize).font('Arial Unicode MS');
  } catch (e) {
  ...
  try {
    doc.fontSize(smallFontSize).font('Arial Unicode MS');
  } catch (e) {
