/**
 * English PDF Service (Refactored)
...
class EnglishPdfService {
  ...
  constructor(useWorkers: boolean = true) {
    ...
    if (this.useWorkers) {
      try {
        ...
        console.log('[PRINT] English PDF Worker Pool initialized');
      } catch (error) {
  ...
  async printTicket(ticketData: TicketData, printerName: string | null = null): Promise<PrintResult> {
    console.log('[PRINT] EnglishPdfService.printTicket called!');
    ...
    try {
      ...
      if (this.useWorkers && this.workerPool) {
        ...
        pdfPath = await this.workerPool.generatePDF(ticketData, 'english');
      } else {
        console.log('[PRINT] Generating PDF synchronously...');
        // Fallback to synchronous generation
        const formattedTicket = formatTicketData(ticketData, false, true);
        const generator = new EnglishTicketGenerator(this.tempDir);
...
export const englishPdfService = new EnglishPdfService(false);
export default englishPdfService;
