...
// Path to downloaded Anek Kannada fonts
const anekFontPath = path.join(__dirname, 'fonts', 'AnekKannada-Regular.ttf');
const anekBoldFontPath = path.join(__dirname, 'fonts', 'AnekKannada-Bold.ttf');
...
function createPDFTicket(ticketData) {
  ...
  const outputPath = path.join(__dirname, 'temp', `ticket_kannada_anek_local_${Date.now()}.pdf`);
  ...
  // Use Anek Kannada font if available, otherwise fallback to Nudi
  let fontPath = anekFontPath;
  if (!fs.existsSync(fontPath)) {
    fontPath = 'C:\\Windows\\Fonts\\Nudi 01 e.ttf';
    console.log('Anek Kannada font not found, using Nudi font as fallback');
  } else {
    console.log('Using Anek Kannada font');
  }
  
  doc.fontSize(16).font(fontPath);
  ...
  // Date and Show info - use Anek Kannada font for Kannada text
  doc.fontSize(normalFontSize).font(fontPath);
  ...
  doc.fontSize(titleFontSize).font(fontPath);
  ...
  doc.fontSize(smallFontSize).font(fontPath);
