...
function createPDFTicket(ticketData) {
  ...
  const outputPath = path.join(__dirname, 'temp', `ticket_kannada_nudi_${Date.now()}.pdf`);
  ...
  // Use Nudi font for Kannada text
  doc.fontSize(16).font(nudiFontPath); // Use the Nudi font for theater name
  ...
  // Date and Show info - use Nudi font for Kannada text
  doc.fontSize(normalFontSize).font(nudiFontPath); // Use Nudi font for Kannada
  ...
  doc.fontSize(titleFontSize).font(nudiFontPath); // Use Nudi font for Kannada
  ...
  doc.fontSize(smallFontSize).font(nudiFontPath); // Use Nudi font for Kannada
  ...
  return new Promise((resolve, reject) => {
    stream.on('finish', () => {
      console.log(`Kannada PDF ticket generated with Nudi font: ${outputPath}`);
      resolve(outputPath);
    });
...
async function testKannadaPDFTicket() {
  try {
    ...
    console.log('Kannada PDF generated successfully with Nudi font!');
