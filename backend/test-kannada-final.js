...
function createPDFTicket(ticketData) {
  ...
  const outputFilePath = path.join(__dirname, 'temp', `test_kannada_final_${Date.now()}.pdf`);
  ...
  // Use the working Nudi font
  const kannadaFont = 'C:\\Windows\\Fonts\\Nudi 01 e.ttf';
  console.log(`Using Kannada font: ${kannadaFont}`);
  ...
  console.log(`Font used: ${kannadaFont}`);
}
